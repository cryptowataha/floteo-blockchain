...
from floteo.cmds.beta_funcs import default_beta_root_path
from floteo.cmds.floteo import cli
from floteo.util.beta_metrics import metrics_log_interval_default, metrics_log_interval_max, metrics_log_interval_min
...
def generate_example_submission_data(beta_root_path: Path, versions: int, logs: int) -> None:
    for version in range(versions):
        ...
        version_path.mkdir()
        chia_blockchain_logs = version_path / "floteo-blockchain"
...
def test_configure_no_beta_config(root_path_populated_with_config: Path) -> None:
    ...
    assert "beta test mode is not enabled, enable it first with `floteo beta enable`" in result.output
...
def test_prepare_submission(
    ...
    if exit_code == 0:
        ...
        with zipfile.ZipFile(submission_file) as zip_file:
            ...
            for version in range(versions):
                chia_blockchain_logs = Path("floteo-blockchain")
