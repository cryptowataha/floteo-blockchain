...
git submodule update --init floteo-blockchain-gui
...
# Other than `floteo-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @floteo packages to make cache slim"
...
# Remove `packages/gui/node_modules/@floteo` because it causes an error on later `electron-packager` command
Remove-Item "@floteo" -Recurse -Force
