...
from floteo.util.default_root import DEFAULT_KEYS_ROOT_PATH
from floteo.util.file_keyring import FileKeyring
from floteo.util.misc import prompt_yes_no
...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ floteo passphrase set # all the cool kids are doing it!"
...
def check_legacy_keyring_keys_present(keyring: LegacyKeyring) -> bool:
    ...
    from floteo.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def __init__(self, keys_root_path: Path = DEFAULT_KEYS_ROOT_PATH, force_legacy: bool = False):
        ...
        """
        from floteo.util.errors import KeychainNotSet
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from floteo.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'floteo passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase?")
            if response:
                from floteo.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'floteo passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from floteo.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    async def migrate_legacy_keyring_interactive(self) -> bool:
        ...
        """
        from floteo.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running

        # Let the user know about the migration.
        if not self.confirm_migration():
            print("Migration aborted, can't run any floteo commands.")
            return False
