...
import stat
import sys
from floteo.util.config import load_config, traverse_dict
from floteo.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `floteo init --fix-ssl-permissions` to fix issues.")
