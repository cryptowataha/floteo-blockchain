...
from floteo.cmds.init_funcs import check_keys, chia_init, chia_full_version_str
from floteo.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from floteo.daemon.keychain_server import KeychainServer, keychain_commands
from floteo.daemon.windows_signal import kill
from floteo.plotters.plotters import get_available_plotters
from floteo.plotting.util import add_plot_directory
from floteo.server.server import ssl_context_for_root, ssl_context_for_server
from floteo.ssl.create_ssl import get_mozilla_ca_crt
from floteo.util.beta_metrics import BetaMetricsLogger
from floteo.util.chia_logging import initialize_service_logging
from floteo.util.config import load_config
from floteo.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
from floteo.util.json_util import dict_to_json_str
from floteo.util.keychain import (
    Keychain,
    passphrase_requirements,
    supports_os_passphrase_storage,
)
from floteo.util.lock import Lockfile, LockfileError
from floteo.util.network import WebServer
from floteo.util.service_groups import validate_service
from floteo.util.setproctitle import setproctitle
from floteo.util.ws_message import WsRpcMessage, create_payload, format_response
...
class WebSocketServer:
    ...
    async def exit(self) -> None:
        ...
        log.info("floteo daemon exiting")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"floteo-blockchain version: {chia_full_version_str()}")
