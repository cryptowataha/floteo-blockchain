...
from floteo.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg
from floteo.cmds.peer_funcs import print_connections
from floteo.cmds.units import units
from floteo.rpc.wallet_rpc_client import WalletRpcClient
from floteo.server.start_wallet import SERVICE_NAME
from floteo.types.blockchain_format.sized_bytes import bytes32
from floteo.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from floteo.util.config import load_config, selected_network_address_prefix
from floteo.util.default_root import DEFAULT_ROOT_PATH
from floteo.util.ints import uint16, uint32, uint64
from floteo.wallet.nft_wallet.nft_info import NFTInfo
...
from floteo.wallet.util.address_type import AddressType, ensure_valid_address
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    print(f"To get status, use command: floteo wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["floteo"])
            ...
            if royalty_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    ...
                    for summary in summaries:
                        divisor = units["floteo"] if summary["asset"] == "XCH" else units["cat"]
                ...
                for asset, requested_amount in total_amounts_requested.items():
                    divisor = units["floteo"] if asset == "XCH" else units["cat"]
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['floteo']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    print(f"Included Fees: {Decimal(offer.bundle.fees()) / units['floteo']}")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["floteo"]
    return "", units["mojo"]
...
async def send_notification(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    amount: uint64 = uint64(Decimal(args["amount"]) * units["floteo"])
    message: bytes = bytes(args["message"], "utf8")
    fee: uint64 = uint64(Decimal(args["fee"]) * units["floteo"])

    tx = await wallet_client.send_notification(address, message, amount, fee)

    print("Notification sent successfully.")
    print(f"To get status, use command: floteo wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
