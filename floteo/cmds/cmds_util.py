...
from floteo.types.mempool_submission_status import MempoolSubmissionStatus
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'floteo wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: Optional[int]) -> Optional[int]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'floteo keys generate' or import a key")
        return None
