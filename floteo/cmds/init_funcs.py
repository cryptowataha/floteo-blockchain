...
from floteo.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from floteo.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'floteo keys generate'")
        return None
...
def chia_init(
    ...
    *,
    ...
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run floteo init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'floteo keys show --show-mnemonic-seed'")
