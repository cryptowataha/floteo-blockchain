...
from floteo.util.beta_metrics import metrics_log_interval_max, metrics_log_interval_min
from floteo.util.chia_logging import get_beta_logging_config
from floteo.util.errors import InvalidPathError
from floteo.util.misc import format_bytes, prompt_yes_no, validate_directory_writable


def default_beta_root_path() -> Path:
    return Path(os.path.expanduser(os.getenv("CHIA_BETA_ROOT", "~/floteo-beta-test"))).resolve()


def warn_if_beta_enabled(config: Dict[str, Any]) -> None:
    if config.get("beta", {}).get("enabled", False):
        print("\nWARNING: beta test mode is enabled. Run `floteo beta disable` if this is unintentional.\n")
