...
from floteo.cmds.cmds_util import get_any_service_client
...
from floteo.util.misc import format_bytes
from floteo.util.misc import format_minutes
from floteo.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total floteo farmed: {amounts['farmed_amount'] / units['floteo']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['floteo']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['floteo']}")
