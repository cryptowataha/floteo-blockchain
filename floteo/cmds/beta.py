...
from floteo.cmds.beta_funcs import (
...
from floteo.util.beta_metrics import metrics_log_interval_default
from floteo.util.config import lock_and_load_config, save_config


def print_restart_warning() -> None:
    print("\nRestart the daemon and any running floteo services for changes to take effect.")
...
def configure(ctx: click.Context, path: Optional[str], interval: Optional[int]) -> None:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if "beta" not in config:
            raise click.ClickException("beta test mode is not enabled, enable it first with `floteo beta enable`")
...
def prepare_submission_cmd(ctx: click.Context) -> None:
    with lock_and_load_config(ctx.obj["root_path"], "config.yaml") as config:
        ...
        if beta_root_path is None:
            raise click.ClickException("beta test mode not enabled. Run `floteo beta enable` first.")
    ...
    chia_blockchain_path = Path(prepare_result / "floteo-blockchain")
