...
from floteo.cmds.coins import coins_cmd
from floteo.cmds.plotnft import validate_fee
...
from floteo.cmds.cmds_util import execute_with_wallet
...
@wallet_cmd.command("send", short_help="Send floteo to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much floteo to send, in XCH", type=str, required=True)
