...
from floteo.protocols.protocol_state_machine import message_requires_reply
from floteo.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from floteo.protocols.shared_protocol import protocol_version
from floteo.server.introducer_peers import IntroducerPeers
from floteo.server.outbound_message import Message, NodeType
from floteo.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from floteo.util.api_decorators import get_metadata
from floteo.util.errors import Err, ProtocolError
from floteo.util.ints import uint16
from floteo.util.network import WebServer, is_in_network, is_localhost
from floteo.util.ssl_check import verify_ssl_certs_and_keys
...
class ChiaServer:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use floteo's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.floteo.net/") as resp:
                    if resp.status == 200:
