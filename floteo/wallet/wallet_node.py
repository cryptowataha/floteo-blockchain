...
from floteo.consensus.blockchain import ReceiveBlockResult
from floteo.consensus.constants import ConsensusConstants
from floteo.daemon.keychain_proxy import (
    KeychainProxy,
    connect_to_keychain_and_validate,
    wrap_local_keychain,
)
from floteo.protocols import wallet_protocol
from floteo.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from floteo.protocols.protocol_message_types import ProtocolMessageTypes
from floteo.protocols.wallet_protocol import (
    CoinState,
    RespondBlockHeader,
    RespondToCoinUpdates,
    RespondToPhUpdates,
)
from floteo.rpc.rpc_server import default_get_connections
from floteo.server.node_discovery import WalletPeers
...
from floteo.types.weight_proof import WeightProof
from floteo.util.chunks import chunks
from floteo.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED, process_config_start_method
from floteo.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from floteo.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from floteo.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from floteo.wallet.util.wallet_sync_utils import (
...
from floteo.wallet.wallet_weight_proof_handler import get_wp_fork_point, WalletWeightProofHandler
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'floteo keys' program.")
            return None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.floteo.net"]),
