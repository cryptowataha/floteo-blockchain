...
from floteo.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk
...
def check_plots(root_path, num, challenge_start, grep_string, list_duplicates, debug_show_memo):
    ...
    if len(plot_manager.no_key_filenames) > 0:
        log.warning(
            ...
            f"farm them, use 'floteo keys' to transfer keys. The pool public keys must be in the config.yaml"
        )
