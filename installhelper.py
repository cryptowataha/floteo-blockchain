...
# Gets the version from the result of "floteo version"
...
def get_chia_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["floteo", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/floteo-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/floteo-blockchain-gui/packages/gui/package.json")
